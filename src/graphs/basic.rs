...
#[derive(Default, PartialEq, Eq, Serialize, Deserialize, Copy, Clone)]
...
mod basic_edge_tests {
    ...
    fn test_debug() {
        ...
        assert_eq!(format!("{edge_map:?}"), "Edges: [(0, [0, 0]), (1, [0, 0]), (4, [0, 0])]");

        let second_edge_map = get_example_non_default_edge_map::<BasicEdge>();
        assert_eq!(format!("{second_edge_map:?}"), "Edges: [(0, [0, 0]), (1, [0, 1]), (2, [2, 1]), (3, [2, 4]), (4, [4, 0]), (5, [4, 5])]");
    }
}
