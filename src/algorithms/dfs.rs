...
impl<N: Node, E: Edge> Graph<N,E> {
    ...
    fn hidden_dfs<'a>(&mut self, start: NodeId, end: NodeId,
        visited_nodes: &mut HashSet<NodeId>, path_stack: &mut Vec<(NodeId, EdgeId)>) -> bool {
            ...
            let vec = self.get_directed_neighbors(&start);
            let mut neighbors = vec.clone().iter().map(|(n, e)| {(n.clone(), e.clone())}).collect::<Vec<_>>();
            neighbors.sort();
    ...
    pub fn dfs(&mut self, start: NodeId, end: NodeId) -> Option<(Vec<(NodeId, EdgeId)>, HashSet<NodeId>)> {
...
mod dfs_tests {
    ...
    #[test]
    fn test_dfs_success() {
        let mut graph = get_example_graph::<BasicNode, BasicEdge>();
